...
exports.scoreIdentity = scoreIdentity;
...
function scoreIdentity(identity, graph) {
    const breakdown = {};
    let total = 0;
    // Privileged authority (edges with wildcard/admin)
    const hasPrivilegedEdge = graph.edges.some((e) => (e.fromIdentityId === identity.id || e.toIdentityId === identity.id) && (e.isAdmin || e.isWildcard));
    ...
    const hasProdPath = graph.executionPaths.some((ep) => ep.identityId === identity.id && ep.isProduction);
    if (hasProdPath) {
    ...
    const hasUnapprovedProdPath = graph.executionPaths.some((ep) => ep.identityId === identity.id && ep.isProduction && !ep.approvalDetected);
    if (hasUnapprovedProdPath) {
        breakdown['missing_approval'] = 15;
        total += 15;
    }
    // Missing evidence/receipt
    const hasNoEvidence = graph.executionPaths.some((ep) => ep.identityId === identity.id && !ep.evidenceDetected);
    if (hasNoEvidence) {
    ...
    const hasBoundaryCrossing = graph.executionPaths.some((ep) => ep.identityId === identity.id && ep.trustBoundaryCrossed);
...
//# sourceMappingURL=trustRiskScoring.js.map