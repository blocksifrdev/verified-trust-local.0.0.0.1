...
async function writeHtmlReport(graph, riskScore, edition, outputDir) {
...
function generateHtmlReport(graph, riskScore, edition) {
    const graphData = graph.toJSON();
    const executiveSummary = (0, executiveSummary_1.generateExecutiveSummary)(graph, riskScore, edition);
    ...
    const allFindingsSorted = [...graphData.findings].sort((a, b) => ['CRITICAL', 'HIGH', 'MODERATE', 'LOW', 'INFO'].indexOf(a.severity) -
        ['CRITICAL', 'HIGH', 'MODERATE', 'LOW', 'INFO'].indexOf(b.severity));
    const privilegedIdentities = Array.from(graph.identities.values()).filter((i) => i.riskTags.includes('admin') || i.riskTags.includes('wildcard') || i.permissions.some((p) => p.includes('*')));
    ...
    const agentIdentities = Array.from(graph.identities.values()).filter((i) => i.identityType === 'agent_identity' || i.identityType === 'mcp_server_identity');
...
<div class="section">
  ...
  <table>
    ...
    <tbody>
      ${Object.entries(sourceSystemCounts).map(([sys, count]) => `<tr><td>${escapeHtml(sys)}</td><td>${count}</td></tr>`).join('')}
    </tbody>
...
<div class="section">
  ...
  <table>
    ...
    <tbody>
      ${graphData.inflectionPoints.slice(0, 15).map((ip) => `<tr>
...
<div class="section">
  ...
  <table>
    ...
    <tbody>
      ${prodPaths.slice(0, 10).map((ep) => `<tr>
...
<div class="section">
  ...
  <table>
    ...
    <tbody>
      ${orphanedIdentities.slice(0, 10).map((i) => `<tr>
...
<div class="section">
  ...
  <table>
    ...
    <tbody>
      ${credFindings.slice(0, 10).map((f) => `<tr>
...
<div class="section">
  ...
  <table>
    ...
    <tbody>
      ${allFindingsSorted.slice(0, 10).map((f, idx) => `<tr>
          <td>${idx + 1}</td>
          <td><span class="badge" style="background:${severityColor(f.severity)}">${escapeHtml(f.severity)}</span></td>
          <td>${escapeHtml(f.title)}</td>
...
<div class="section">
  ...
  <div class="upgrade-box">${escapeHtml(edition_1.UPGRADE_MESSAGE)}</div>` : ''}
</div>
