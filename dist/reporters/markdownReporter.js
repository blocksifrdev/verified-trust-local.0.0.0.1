...
async function writeMarkdownReport(graph, riskScore, outputDir) {
...
function generateMarkdownReport(graph, riskScore) {
...
|------|------|--------|-------|-----------|
${graphData.identities.slice(0, 20).map((i) => `| \`${i.name.replace(/\|/g, '\\|')}\` | ${i.identityType} | ${i.sourceSystem} | ${i.owner ?? '_None_'} | ${i.riskTags.join(', ') || '-'} |`).join('\n')}
